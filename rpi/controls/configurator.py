...
class Configurator(object):
   ...
   def map_slots_to_bottles(self):
      ...
      if not os.path.isfile(pickled_reg_fname):
         ...
         menu = self.build_slot_mapping_menu()
         ...
         with open(pickled_reg_fname, "wb") as pickle_file:
            pickle.dump(reg, pickle_file, pickle.HIGHEST_PROTOCOL)
      else:
         with open(pickled_reg_fname, "rb") as pickle_file:
            reg = pickle.load(pickle_file)

   def build_main_menu(self):
      # Build menu
      return Menu(None, [
         MenuItem(Alcohol, self.reg.get_all, name="By Alcohol"),
         MenuItem(Mixer  , self.reg.get_all, name="By Mixer")
      ])

   def build_slot_mapping_menu(self):
      return Menu(None,
            map(lambda i: MenuItem(i, None), self.reg.get_all_ingredients()))
